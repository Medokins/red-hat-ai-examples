...
from kfp_components.components.training.automl.autogluon_models_selection import (
    models_selection,
)
...
def autogluon_tabular_training_pipeline(
    ...
    **AutoGluon Ensembling Approach:**
    ...
    selection_task.set_cpu_request("4").set_memory_request("4Gi")

    # Stage 2: Model Refitting
    # Refit each top model on the full training dataset

    with dsl.ParallelFor(
        items=selection_task.outputs["top_models"], parallelism=2
    ) as model_name:
        ...
        refit_full_task.set_cpu_request("4").set_memory_request("4Gi")
