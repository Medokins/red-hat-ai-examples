...
def autogluon_models_full_refit(
    ...
    extra_train_df = (
        pd.read_csv(extra_train_data_path) if extra_train_data_path else None
    )
    ...
    predictor_clone = predictor.clone(
        path=output_path / "predictor", return_clone=True, dirs_exist_ok=True
    )
    ...
    with open(os.path.join(notebooks.path, notebook_file), encoding="utf-8") as f:
    ...
    sample_row_formatted = [
        {col: value for col, value in row.items() if col != predictor.label}
        for row in sample_row_list
    ]
