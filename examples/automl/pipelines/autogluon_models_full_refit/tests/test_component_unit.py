...
class TestAutogluonModelsFullRefitUnitTests:
    ...
    def test_full_refit_with_valid_model(
        ...
        mock_predictor_clone.feature_importance.return_value = mock.MagicMock(
            to_dict=lambda: feature_importance_dict
        )
        ...
        assert (
            mock_model_artifact.metadata["context"]["label_column"]
            == mock_predictor.label
        )
        assert (
            mock_model_artifact.metadata["context"]["metrics"]["test_data"]
            == eval_results
        )
        ...
        assert (
            mock_model_artifact.metadata["context"]["location"]["model_directory"]
        ...
        assert (
            Path(call_kw["path"])
            == Path(model_output_dir) / "LightGBM_BAG_L1_FULL" / "predictor"
        )
        ...
        mock_predictor_clone.delete_models.assert_called_once_with(
            models_to_keep=["LightGBM_BAG_L1"]
        )

        # Verify set_model_best was called with correct model
        mock_predictor_clone.set_model_best.assert_called_once_with(
            model="LightGBM_BAG_L1_FULL", save_trainer=True
        )
        ...
        notebook_path = (
            ...
            / "LightGBM_BAG_L1_FULL"
            / "notebooks"
            / "automl_predictor_notebook.ipynb"
        )
    ...
    @mock.patch("pandas.read_csv")
    @mock.patch("autogluon.tabular.TabularPredictor")
    def test_full_refit_without_extra_train_data(
        self, mock_predictor_class, mock_read_csv, mock_notebooks, tmp_path
    ):
        ...
        mock_predictor_clone.refit_full.assert_called_once_with(
            model="LightGBM_BAG_L1", train_data_extra=None
        )
    ...
    @mock.patch("pandas.read_csv")
    @mock.patch("autogluon.tabular.TabularPredictor")
    def test_full_refit_handles_file_not_found_test_dataset(
        self, mock_predictor_class, mock_read_csv, mock_notebooks
    ):
    ...
    @mock.patch("pandas.read_csv")
    @mock.patch("autogluon.tabular.TabularPredictor")
    def test_full_refit_handles_refit_failure(
        self, mock_predictor_class, mock_read_csv, mock_notebooks
    ):
        ...
        mock_predictor_clone = mock.MagicMock()
        mock_predictor.clone.return_value = mock_predictor_clone
        mock_predictor_clone.refit_full.side_effect = ValueError(
            "Model not found in predictor"
        )
        mock_predictor_class.load.return_value = mock_predictor
    ...
    def test_full_refit_operations_called_in_correct_order(
        ...
        mock_predictor = mock.MagicMock()
        mock_predictor_clone = mock.MagicMock()
        mock_predictor.clone.side_effect = lambda **kw: (
            call_order.append("clone"),
            mock_predictor_clone,
        )[1]
        mock_predictor_clone.delete_models.side_effect = lambda **kw: call_order.append(
            "delete_models"
        )
        mock_predictor_clone.refit_full.side_effect = lambda **kw: call_order.append(
            "refit_full"
        )
        mock_predictor_clone.set_model_best.side_effect = (
            lambda **kw: call_order.append("set_model_best")
        )
        mock_predictor_clone.save_space.side_effect = lambda: call_order.append(
            "save_space"
        )
        mock_predictor_clone.evaluate.side_effect = lambda df: (
        ...
        mock_predictor_class.load.side_effect = lambda path: (
    ...
    def test_full_refit_writes_confusion_matrix_for_classification(
        ...
        mock_confusion_matrix.return_value = mock.MagicMock(
            to_dict=lambda: confusion_matrix_dict
        )
    ...
    def test_full_refit_rejects_empty_predictor_path(self, mock_notebooks):
        ...
        mock_model_artifact.metadata = {}
        with pytest.raises(
            TypeError, match=r"predictor_path must be a non-empty string\."
        ):
            autogluon_models_full_refit.python_func(
                model_name="LightGBM_BAG_L1",
                test_dataset=mock_test_dataset,
    ...
    def test_full_refit_rejects_invalid_sampling_config_type(self, mock_notebooks):
        ...
        mock_model_artifact.metadata = {}
        with pytest.raises(
            TypeError, match=r"sampling_config must be a dictionary or None\."
        ):
            autogluon_models_full_refit.python_func(
                model_name="LightGBM_BAG_L1",
                test_dataset=mock_test_dataset,
    ...
    def test_full_refit_rejects_invalid_split_config_type(self, mock_notebooks):
        ...
        mock_model_artifact.metadata = {}
        with pytest.raises(
            TypeError, match=r"split_config must be a dictionary or None\."
        ):
            autogluon_models_full_refit.python_func(
                model_name="LightGBM_BAG_L1",
                test_dataset=mock_test_dataset,
    ...
    def test_full_refit_rejects_invalid_model_config_type(self, mock_notebooks):
        ...
        mock_model_artifact.metadata = {}
        with pytest.raises(
            TypeError, match=r"model_config must be a dictionary or None\."
        ):
            autogluon_models_full_refit.python_func(
                model_name="LightGBM_BAG_L1",
                test_dataset=mock_test_dataset,
